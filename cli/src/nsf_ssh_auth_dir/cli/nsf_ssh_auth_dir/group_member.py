...
@cli_ssh_user_id_argument()
@pass_cli_ctx
def add(ctx: CliCtx, ssh_group_id: str, ssh_user_id: Optional[str]) -> None:
    """Add a *ssh user* to a *ssh group*."""
    user_id = ensure_ssh_user_id_or_fallback_or_fail(
        ssh_user_id, ctx.user_id)

    try:
        ctx.repo.groups[ssh_group_id].add_member_by_id(user_id)
...
@cli_ssh_group_member_id_argument()
@pass_cli_ctx
def rm(ctx: CliCtx, ssh_group_id: str, ssh_group_member_id: str) -> None:
    """Remove a *ssh user* from a *ssh group*."""
    try:
        ctx.repo.groups[ssh_group_id].rm_member_by_id(ssh_group_member_id)
    except SshGroupsRepoAccessError as e:
        raise CliError(str(e)) from e
