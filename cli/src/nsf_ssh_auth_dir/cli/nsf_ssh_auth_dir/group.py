...
@pass_cli_ctx
def add(ctx: CliCtx, ssh_group_id: str) -> None:
    """Add a new *ssh group*."""
    try:
        ctx.repo.groups.add(ssh_group_id)
...
@cli_ssh_group_id_argument()
@pass_cli_ctx
def rm(ctx: CliCtx, ssh_group_id: str) -> None:
...
def authorize(
    ...
    for du in dus:
        try:
            du.authorize_group_by_id(ssh_group_id)
...
def deauthorize(
    ...
    for du in dus:
        try:
            du.deauthorize_group_by_id(ssh_group_id)
