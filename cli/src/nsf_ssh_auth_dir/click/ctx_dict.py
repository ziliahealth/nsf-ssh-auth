...
def find_ctx_dict_instance(
        ctx: click.Context, at_key: str, instance_type: Type[_T]) -> Optional[_T]:
...
def ensure_ctx_obj_is_dict(obj: Dict[str, _T]) -> Dict[str, _T]:
...
def ensure_ctx_obj_is_dict_or_unspecified(
        obj: Optional[Dict[str, _T]]) -> Dict[str, _T]:
    if obj is None:
        return {}
...
def mk_ctx_dict_obj(
        obj: Optional[Dict[str, _T]] = None) -> Dict[str, _T]:
